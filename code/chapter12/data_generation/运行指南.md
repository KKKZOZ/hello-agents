...
**参数说明**：
- `30` - 生成 30 道题目
- `3.0` - 每次生成间隔 3 秒（推荐 2-3 秒）

**说明**：
- 使用 AIME 2025 年真题作为评估参考
- 数据集来源：math-ai/aime25（JSONL 格式）

**预计耗时**：
- 生成 30 道题：约 15-30 分钟（取决于 API 速度）
- LLM Judge 评估：约 10-15 分钟
- Win Rate 评估：约 5-10 分钟
- **总计**：约 30-55 分钟
...
- 基本信息（生成时间、题目数量等）
- 数据生成统计（主题分布、答案分析）
- LLM Judge 评估结果（总体评分、各维度评分）
- Win Rate 评估结果（胜率统计、对比分析）
- 综合结论和改进建议
